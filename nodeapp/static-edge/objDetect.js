...
const apiServer = 'http://localhost:8089/style.css'; //the full TensorFlow Object Detection API server url
...
function postFile(file) {
    ...
    xhr.onload = function () {
        if (this.status === 200) {
            ...
            edgeDataArray.push(totalTime)
            document.getElementById('max-resp-time').innerHTML=Math.max(...edgeDataArray).toFixed(2)
            document.getElementById('min-resp-time').innerHTML=Math.min(...edgeDataArray).toFixed(2)
            document.getElementById('avg-resp-time').innerHTML=(edgeDataArray.reduce((a,b) => a + b, 0) / edgeDataArray.length).toFixed(2)
            edgeChart.options.data[0].dataPoints.push({ y: totalTime });
            edgeChart.render();             
