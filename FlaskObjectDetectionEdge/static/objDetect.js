...
v = document.getElementById('edgeVideo');
//for starting events
let isPlaying = false,
  gotMetadata = false;
...
let drawCanvas = document.getElementById('edge-canvas');
...
function drawBoxes(objects) {
  ...
  objects
    ...
    .forEach((object) => {
      ...
      drawCtx.fillText(
        object.label + ' - ' + Math.round(object.probability * 100) + '%',
        x + 5,
        y + 20
      );
...
function postFile(file) {
  ...
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      // let  rttProcessingTime= totalTime-processingTime;
      ...
      document.getElementById('processing-max-resp-time').innerHTML =
        Math.max(...processingTimeArray).toFixed(2) + ' ms';
      document.getElementById('processing-min-resp-time').innerHTML =
        Math.min(...processingTimeArray).toFixed(2) + ' ms';
      document.getElementById('processing-avg-resp-time').innerHTML =
        (
          processingTimeArray.reduce((a, b) => a + b, 0) /
          processingTimeArray.length
        ).toFixed(2) + ' ms';
      ...
      document.getElementById('overall-max-resp-time').innerHTML =
        Math.max(...edgeDataArray).toFixed(2) + ' ms';
      document.getElementById('overall-min-resp-time').innerHTML =
        Math.min(...edgeDataArray).toFixed(2) + ' ms';
      document.getElementById('overall-avg-resp-time').innerHTML =
        (
          edgeDataArray.reduce((a, b) => a + b, 0) / edgeDataArray.length
        ).toFixed(2) + ' ms';
